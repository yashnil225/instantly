...
type Step = 'source' | 'csv' | 'sheets' | 'manual' | 'mapping'
...
export function ImportLeadsModal({ open, onOpenChange, onImportSuccess }: ImportLeadsModalProps) {
    ...
    const processFile = (file: File) => {
        ...
        Papa.parse(file, {
            header: true,
            skipEmptyLines: true,
            preview: 5, // Just to get headers and sample
            complete: (results) => {
                if (results.meta.fields) {
                    setCsvHeaders(results.meta.fields)
                    if (results.data.length > 0) {
                        const firstRow = results.data[0] as any
                        // Map values safely using the headers order
                        const sample = results.meta.fields.map(h => firstRow[h] || '')
                    ...
                    results.meta.fields.forEach(header => {
                        ...
                        else if (lower.includes('company')) initialMapping[header] = 'companyName'
                        else if (lower.includes('web')) initialMapping[header] = 'website'
                        else if (lower.includes('phone')) initialMapping[header] = 'phone'
            ...
            try {
                const mappedLeads = rawData.map((row: any) => {
                    ...
                    Object.keys(row).forEach((header) => {
                        ...
                        if (targetField && targetField !== 'skip' && targetField !== 'custom') {
                            lead[targetField] = row[header]
                        }
                    })
                    return lead
    ...
    const handleImportFromSheets = async () => {
        ...
        try {
            ...
            const sample = headers.map(h => firstRow[h] || '')
            setFirstRowSample(sample)

            // Auto-guess mapping (Reuse logic? Yes, copy for now or extract function)
            const initialMapping: Record<string, string> = {}
                ...
                {step === 'csv' && (
                    <div className="p-8 flex flex-col h-full flex-1 justify-center items-center">
                        ...
                        >
                            ...
                            <span className="text-sm font-bold text-gray-500 tracking-widest">CLICK TO UPLOAD</span>
                            <span className="text-xs text-gray-600 mt-2">or drag and drop</span>
