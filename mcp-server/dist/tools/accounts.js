...
export async function handleGetAccount(args) {
...
export async function handleUpdateWarmup(args) {
    ...
    if (updates.enabled !== undefined)
        data.warmupEnabled = updates.enabled;
    if (updates.dailyLimit !== undefined) {
        data.warmupDailyLimit = updates.dailyLimit;
        data.warmupMaxPerDay = updates.dailyLimit;
    }
    if (updates.dailyIncrease !== undefined)
        data.warmupDailyIncrease = updates.dailyIncrease;
    if (updates.replyRate !== undefined)
        data.warmupReplyRate = updates.replyRate;
    if (updates.poolOptIn !== undefined)
        data.warmupPoolOptIn = updates.poolOptIn;
